...
def mclust_R(adata, num_cluster, modelNames='EEE', used_obsm='emb_pca', random_seed=2020):
    ...
    #modification: adata.obs['mclust'] --> adata.obs[f'mclust_{num_cluster}']
    adata.obs['mclust'] = mclust_res
    adata.obs['mclust']  = adata.obs[f'mclust'].astype('int')
    adata.obs['mclust']  = adata.obs[f'mclust'].astype('category')
    return adata
...
def clustering(adata, n_clusters_list=7, radius=50, key='emb_pca', method='mclust', start=0.1, end=3.0, increment=0.01, refinement=False):
    ...
    n_clusters_list : int or list(int), optional
        The number of clusters. The default is 7.
    radius : int, optional
        The number of neighbors considered during refinement. The default is 50.
    key : string, optional
        The key of the learned representation in adata.obsm. The default is 'emb_pca'.
...
def search_res(adata, n_clusters_list, method='leiden', use_rep='emb_pca', start=0.1, end=3.0, increment=0.01):
    # n_clusters --> n_clusters_list
    '''\
    ...
    adata : anndata
        AnnData object of spatial data.
    n_clusters_list : int or list(int) 
    ...
    for res in sorted(list(np.arange(start, end, increment)), reverse=True):
        ...
        if count_unique in n_clusters_list: #modification: count_unique == n_clusters
            print(f'save the resolution ({res})') #add: check 
            res_list.append((res, count_unique)) #add
            n_clusters_list.remove(count_unique) #remove the number of clusters from the list when the resolution associated is identified
            #label = 1
        if not n_clusters_list: #add
            break

    #assert label==1, "Resolution is not found. Please try bigger range or smaller step!." 
    if n_clusters_list: #add: if the n_clusters_list is not empty (no resolution associated with the number of clusters of interest)
        print("Resolution is not found. Please try bigger range or smaller step!.")
